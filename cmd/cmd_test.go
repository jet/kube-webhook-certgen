...
func TestGeneralWiring(t *testing.T) {
	...
	// Regular post behaviour
	exitCode = post(m)
	if exitCode != 0 {
		t.Fatal("Expected 0 exit code")
	}

	// No secret should exit with 1 in pre flow
	...
	// No secret should exit with 1 in post flow
	cfg.secretName = notExists
	exitCode = post(m)
	if exitCode != 1 {
		t.Fatal("Expected 1 exit code")
	...
	// Pre flow - single hook failure should still be ok
	cfg.secretName = goodSecret
	...
	// Post flow - single hook failure should fail
	exitCode = post(m)
	if exitCode != 1 {
		t.Fatal("Expected 1 exit code")
	}
